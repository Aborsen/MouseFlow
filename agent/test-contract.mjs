...
group('пачку режет одно правило, и его читают оба драйвера');
{
  ...
  check('в пачку идут только то, что не целится в картинку',
    /const BATCHABLE = new Set\(\['type_text', 'press_key', 'wait', 'clipboard_read', 'clipboard_write'\]\)/.test(brain));
  ...
  check('а после ожидания, активации, открытия и наведения - ничего',
    /const TERMINAL = new Set\(\['wait', 'activate_window', 'hover', 'open_url', 'open_app'\]\)/.test(brain));
