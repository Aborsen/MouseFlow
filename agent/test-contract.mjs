...
group('Enter делает то же, что главная кнопка');
{
  const create = read('web/src/features/create/CreateView.tsx');
  ...
  check('и его же спрашивает кнопка', /\) : planned \? \([\s\S]{0,400}Run it/.test(code));
  check('без плана Enter планирует, а не запускает',
    /if \(planned \|\| ev\.metaKey \|\| ev\.ctrlKey\) void send\(\);\s*else void makePlan\(\);/.test(code));
  /* Быстрый путь остался - он просто перестал быть тем, что происходит само. */
  check('быстрый путь есть и назван под полем',
    /runs it without a plan/.test(create) && /hostOS\(\) === 'macos'/.test(code));
  /* Прогон по-прежнему берёт ИМЕННО одобренный план: без этого чекпоинты не сработают ни разу. */
  check('и одобренный план доезжает до цикла',
