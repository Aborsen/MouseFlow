...
group('слишком большая запись уезжает частями, а не отказом по второму разу');
{
  ...
  check('импорт тоже режет слишком большой файл, а не упирается в тот же потолок',
    view.includes('const fit = partsToFit({ rec, events: rec.events, health })'));
  check('и говорит, что импортированное стало частями',
    view.includes('too big for a single row'));

  /* ЧАСТИ, КОТОРЫХ НЕ ВИДНО, - вторая половина того же живого случая. Первая версия сплита толкала строки на
   * аккаунт и останавливалась: таблица записей исключает части сессий НАМЕРЕННО, а полоса сессий читает
   * state.sessions, куда никто не написал. В базе лежали обе части (проверено запросом: 97 466 + 57 509 =
   * все 154 975 событий), на экране - ничего, и человек справедливо сказал «по факту ничего нет». */
  check('после сплита сессия пишется в леджер - иначе части доехали и пропали с глаз',
    view.includes('...split.map((x) => deliveredSession(x.session))')
      && view.includes('...(prev.sessions as Session[]).filter((s) => s.id !== delivered.id)'));
  check('и оба пути метят части доехавшими одним помощником, который не разойдётся',
    src.includes('export const deliveredSession') && view.split('deliveredSession(').length >= 3);
  /* Оставленный оригинал - не дубль: реконсайлер перепосылает всякую локальную запись без штампа на каждом
   * такте, отказ «too large» не терминален, а штамп успеха ставится на весь батч - девять мегабайт, которые
   * никогда не влезут, отравляли бы синхронизацию всего остального. */
  check('локальный оригинал после сплита убирается - он отравлял бы каждый батч синхронизации',
    view.includes('recordings: prev.recordings.filter((r) => r.id !== rec.id)')
      && view.includes('recordings: prev.recordings.filter((r) => !split.some((x) => x.rec.id === r.id))'));
  check('панель сразу открывает первую часть, а не остаётся на id, которого больше нет',
    view.includes('setViewing(delivered.parts[0]?.id ?? null)'));
  check('и обе подписи говорят, ГДЕ искать части - в полосе сессий',
    view.split('Sessions strip').length >= 3);
}
