...
group('кейс можно привязать к машине, и до миграции это ничего не ломает (пункт 7, часть 2)');
{
  ...
  /* И МИГРАЦИЯ СУЩЕСТВУЕТ, НАПИСАНА И ГОВОРИТ, ЧТО НЕ ПРИМЕНЕНА. Файл без этой записки следующий
   * читатель применил бы, решив, что так и надо. */
  ...
  check('и в ней сказано, что она не применена и почему',
    /НЕ ПРИМЕНЕНА/.test(said022) && /только по явному разрешению владельца/.test(said022));
