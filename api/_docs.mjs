...
export const DOC_SYSTEM = [
  ...
  '- TYPED TEXT IS NOT RECORDED. The recorder stores that a key was pressed and when, and the name of a key',
  '  that cannot spell anything (Enter, Tab, a Ctrl or Command chord) - never a character and never the words.',
  ...
  '- WHERE IT HAPPENED IS PART OF THE INSTRUCTION. A stretch whose `where` carries a `url` was on that',
  '  page, and a procedure that says "open the order search" without saying where is a procedure somebody',
  '  cannot follow. Name the site the first time each one appears, as a Markdown link - [2Checkout]',
  '  (https://secure.2checkout.com/cpanel/reports.php) - using the url exactly as given. Do not invent a',
  '  url, do not shorten one to its host, and do not repeat the link on every step.',
  '- The urls carry no query string: it was removed before you saw it, because a document gets exported and',
  '  sent on and a query string is where session tokens live. So a link goes to the page, not to the',
  '  particular record that was open. Say so once, in the limitations, rather than beside every link.',
  '',
  ...
  '  ## Where this runs - every site and application the work moved through, each as a link where the',
  '  transcript gives a url, in the order they were first used. This section is what makes the document',
  '  usable by somebody who has not seen the screen.',
