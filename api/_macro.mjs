...
export function dropOwnTail(events, ownTitle) {
  ...
  /* НАШЛИ ОСТАНОВКУ - ИЛИ НЕ ТРОГАЕМ НИЧЕГО. Сначала считаем, где кончается наш хвост, и только потом
   * режем: движения снимаются ТОЛЬКО как дорога к найденной кнопке.
   *
   * Порядок проверки такой потому, что порядок событий такой: [работа] [дорога к кнопке] [нажали]
   * [отпустили] [возможное дрожание]. Значит от конца: пропустить дрожание, увидеть пару, и только
   * убедившись, что нажатие было по НАШЕМУ окну, снять и пару, и дорогу к ней.
   *
   * ПОЧЕМУ НЕ БЕЗУСЛОВНО. Первая версия снимала хвост движений всегда, и это видно на живой записи
   * (rnbf8qji9): запись остановили из чата - кнопку никто не нажимал, - а правило сняло бы двенадцать
  ...
  /* И дорога к кнопке - за ней: сама по себе она безвредна, но запись, кончающаяся дорогой в угол экрана,
   * при повторе туда же и уводит курсор, и следующая догадка «где мышь» считается от угла. */
  while (end > 0 && move(list[end - 1])) end--;
  return { events: list.slice(0, end), dropped: list.length - end };
}

/* ЕСТЬ ЛИ В ЗАПИСИ ХОТЬ ЧТО-ТО, ЧТО МОЖНО СЫГРАТЬ.
 *
 * Найдено на живой записи (rq2pjkuxw, 61 событие, один клик): после отреза остановки от неё осталось РОВНО
 * ОДНО событие - «Focus», которое агент пишет при смене переднего окна. Пустой список это не пустой список,
 * и `if (!events.length)` его пропускал: на аккаунт уезжала запись из одной пометки, которая при повторе
 * не делает ничего и отчитывается как «1 событие сыграть не удалось».
 *
 * «Focus» - это ЗАМЕТКА, а не действие, и так к нему относится сам агент: на повторе он попадает в
 * `_unplayable` вместе с «Key Down», потому что играть в нём нечего. Значит запись, в которой не осталось
 * ничего кроме заметок, - это запись, в которой не осталось ничего.
 *
 * Только «Focus». Движения не заметки: они сдвигают курсор, и запись из одних движений воспроизводима.
 * «Key Down» тоже не заметка: содержимое не записано намеренно, но ВРЕМЯ набора записано, и повтор его
 * выжидает - это и есть та запись.
 *
 * @param {{action?: string}[]} events
 * @returns {boolean} есть ли хоть одно событие, которое не является пометкой
 */
export function hasPlayable(events) {
  if (!Array.isArray(events)) return false;
  return events.some((event) => String((event && event.action) || '') !== 'Focus');
}
