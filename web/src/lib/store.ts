...
export interface RecordedEvent {
  ...
  context?: {
    ...
    modifiers?: string;
    /* ГДЕ БЫЛИ ОКНО И НАЗВАННЫЙ ЭЛЕМЕНТ - в экранных пикселях, на момент клика. Пишет агент, который
     * объявляет canAnchor; у записей старше него якоря нет, и отсутствие здесь - ответ: повтор тогда играет
     * записанную точку и говорит об этом словами. Восемь чисел на проводе, две рамки здесь.
     *
     * Форма та же, что у MacroEvent.context в api/_macro.d.mts, по той же причине, что и у modifiers. */
...
export interface Recording {
  ...
  /* КОГДА события происходили - шестнадцать счётчиков по равным частям длины записи.
   *
   * Приходит С АККАУНТА (api/sync.js, из flow_digest) и здесь не считается. Считать её можно было бы прямо
   * в лестнице освобождения ниже, у которой события ещё в руках, - но тогда одна картинка выводилась бы
   * двумя способами, а строкам, чьи события выложены давно, это всё равно не помогло бы: их события здесь
   * уже не лежат.
   *
   * Есть только у записи, которая не может нарисовать себя сама. Отсутствует, пока дайджест записи не
   * посчитан, - и это НЕ то же, что шестнадцать нулей: см. web/src/components/Signal.tsx. */
  shape?: number[];
