...
| `db/NNN_*.sql` | Migrations, applied by `npm run migrate` (reads `.env.local`) | Next free number: **023** (`022_queue_machine.sql` is the latest, **written and not yet applied**). Applying to production is the owner's decision — ask, never run unasked, and **never reuse a number**: `migrate.mjs` walks them in name order and records what it ran by name, so twins mean the second one silently never runs. |
...
is being framed as **two products on one engine** — a documentation product (record work → a process
document, read-only) and a QA product (goal loop + checks + verdicts) — and **coordinate replay is frozen,
not deleted**: four fixes in one day were all one class (a recorded point is a claim about a screen that no
longer exists), so investment stops there. Second, **item 4 (hybrid replay) is proposed superseded** by a
memory of applications: a fact about an application is learned once and applied on every run, instead of the
model repairing a step at run time. The owner has not struck item 4; treat it as parked until they do.
...
tonight), and the three new tools do not include running or scheduling — `mouseflow_run` and
`mouseflow_schedule` take `case` where they take `skill`.
