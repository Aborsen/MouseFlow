...
--
-- НЕ ПРИМЕНЕНА. Тот же стоячий порядок, что у 022: против продакшена - только по явному разрешению
-- владельца, потом `npm run migrate -- --list`. Код, который читает эту таблицу (`memoryForOpen`,
-- `api/_memory.mjs`, за флагом `MEMORY_LIVE`), сегодня работает БЕЗ неё - карта записей пустая, потому
-- что взять её пока неоткуда, и ни один прогон от этого не ломается. Ход первым, кто это применит:
-- `npm run migrate`, затем `npm run migrate -- --list`.
--
